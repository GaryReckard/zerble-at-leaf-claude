...
- **Star Power.** Rare hidden floating star, marked by a thin pillar of light visible from across the festival. Catch it and Zerble enters a 15-second buff: invincible to damage, polygons cycling through silvery rainbow via a shared `onBeforeCompile` shader patch, fast jaunty 160 BPM loop overrides the music bus, and every NPC within 25m falls in love and starts spawning smiles continuously. Big crowd-plow energy. Full design in [`.claude/star-power-design.md`](.claude/star-power-design.md) — spawn rules, state machine, rainbow shader strategy, audio swap, love-magnet mechanics, edge cases (trip-vs-star priority, chunk unload), and the 7-step build order.
